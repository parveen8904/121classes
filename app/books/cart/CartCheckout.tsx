...
export default function CartCheckout({ configured }: { configured: boolean }) {
  ...
  // NOTHING REACHES THE GATEWAY UNTIL THIS IS NON-NULL — see
  // CheckoutAddressStep. Any edit sets it back to null, so the details paid
  // against are always the details that were looked at.
    ...
    <div style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <button className="btn block" type="button"
          disabled={busy || !configured || total <= 0 || !confirmed}
          onClick={checkout}>
          {busy ? "Starting…" : confirmed
            ? `🔒 Pay ₹${total.toLocaleString("en-IN")} & place order →`
            : "Confirm your details above to pay"}
        </button>
        {problem.length > 0 && (
