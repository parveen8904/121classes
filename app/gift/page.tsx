...
export default async function GiftPage() {
  ...
  const { data: subjects } = await svc
    .from("subjects")
    .select("id, title, gold_price_inr, validity_months, gold_slabs, batch_months, batch_price_inr, courses(title)")
    .order("order_index");
    ...
    <main>
      <section className="container" style={{ paddingTop: 30, paddingBottom: 60, maxWidth: 640 }}>
        ...
        <GiftForm
          ...
          subjects={(subjects ?? []).map((s) => ({ id: s.id as string, title: s.title as string, course: (s as { courses?: { title?: string } | null }).courses?.title ?? "", gold: (s as { gold_price_inr?: number | null }).gold_price_inr ?? null, validityMonths: (s as { validity_months?: number | null }).validity_months ?? 12, goldSlabs: (s as { gold_slabs?: unknown }).gold_slabs ?? null, batchMonths: (s as { batch_months?: number | null }).batch_months ?? null, batchPriceInr: (s as { batch_price_inr?: number | null }).batch_price_inr ?? null }))}
