...
import { checkQuota, consumeQuota } from "@/lib/entitlements";
...
export async function askSubjectDoubt(input: {
...
}): Promise<{ ok: boolean; answer: string | null; limited?: boolean; upgrade?: boolean; used?: number; limit?: number }> {
  ...
  // Plan quota: free students get a fixed number of AI queries (e.g. 10), then
  // must upgrade. Paid plans are unlimited.
  const q = await checkQuota(user.id, "ask_query");
  if (!q.allowed) return { ok: true, answer: null, upgrade: true, used: q.used, limit: q.limit };
  ...
  // Count this query toward the plan quota (no-op for unlimited plans).
