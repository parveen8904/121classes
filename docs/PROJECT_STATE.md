...
## 26. Phase: founder backlog items #1–#4 (built — 2026-06-14)
- **Admin Courses redesign** (`/admin/courses`): clean "Manage courses" list, right-aligned "＋ New course" expander, per-row "✏️ Edit course details" expander (uses existing `updateCourse`). Search/filter removed. New `summary.as-btn` CSS helper.
- **Classes under topics**: `full_class_video` is now a "Class" = video + PDF + discussion. `sectionTypes.ts` gives it a `pdf_url` field; new `PdfUpload` component (public `media` bucket, folder `materials`) wired into `SectionForm`. Topic admin shows a prominent "🎓 Add a class" (`defaultType` prop on SectionForm); other section types behind an expander. Student topic page renders the class video + "📄 Class notes (PDF)" + "💬 Discussion"; `/learn/section/[id]?view=discussion` renders the per-class discussion board (any section, RLS-gated).
- **Flexible Gold validity**: students pick Gold validity from admin-set months (`site_settings.gold_validity_options`, default `1,2,3,6,12,18,24`) or a custom number; price scales from the subject's base `gold_price_inr`/`validity_months` (`payActions` clamps 1..60). Silver stays flat. Admin Plans page edits the options (`setGoldValidityOptions`).
- **Announcements/amendments → landing**: landing (`force-dynamic`) already renders published `announcements`; added a prominent "📌 Latest ICAI amendments" strip (derived from `kind='amendment'`). Admin Announcements already supports the `amendment` kind → posting one shows on the landing immediately.

## 27. Desktop app + apps + backlog features (built — 2026-06-14)
- **Encrypted downloadable classes (desktop app):** see §26 / `docs/DESKTOP_APP_PLAN.md`. Mac `.dmg` is **signed + notarized + universal** (Developer ID "Aldine Ventures Private Limited", Team `32W63QKXH8`; notarize creds in keychain profile `ca-notary`). Windows `.exe` x64 unsigned (needs a Windows cert). Both hosted on Bunny Storage; URLs in `site_settings.app_url_mac/windows`. One real class registered (`protected_videos`): "CA Final FR — IND AS 115".
  - **Live installer URLs (v2, verified 2026-06-14, HTTP 200 + size-matched):** Mac `https://ca-classes.b-cdn.net/CA-Classes-Mac-v2.dmg` (178,355,930 B); Windows `https://ca-classes.b-cdn.net/CA-Classes-Windows-v2.exe` (78,351,170 B). `site_settings.app_url_mac` / `app_url_windows` point here. Uploaded directly via `curl -T` PUT to `sg.storage.bunnycdn.com/ca-classes/` (201 = ok). See `docs/HANDOVER.md` for the full operator runbook. **Desktop base URL must be `https://www.121caclasses.com`** (apex 308-redirects → breaks CORS).
- **Installable PWA + install helper:** `app/manifest.ts`, `public/sw.js`, `RegisterSW`, `app/apple-icon.tsx`, `public/icon.svg`. `/install` (device-detecting one-tap) and `/help` (per-platform guide). Landing "Get the app" buttons (Web/Mac/Win/iPhone/Android) driven by `site_settings.app_url_*`.
- **AI tests from transcripts (#1):** `lib/ai.ts` `generateMcqs()` + `generateSubjectiveQuestions()` — pre-generate questions ONCE from a pasted transcript, store in `mcq_questions`/`subjective_questions`, serve statically (no per-student tokens). Admin MCQ + Subjective pages have a "🤖 Generate from transcript" expander (graceful note when `ANTHROPIC_API_KEY` absent). Model = existing `callClaude` default (`claude-sonnet-4-6`, cost-appropriate).
- **Live-class scheduling (#2):** migration `0012` `live_sessions` (standalone classes, audience/"for whom", time, join/recording, public-read RLS). Admin → Live has a standalone scheduler; student `/live` merges them; landing shows an "Upcoming live classes" teaser.
- **Bulk notifications (#3):** `/admin/notifications` composer. `lib/notify.ts` adds `sendTelegramChannel()` (one post → all channel members, scale-safe) + `telegramConfigured()`. Email broadcasts to students in batches of 500/click (Mailgun). Needs `TELEGRAM_BOT_TOKEN` (+ bot as channel admin) / `MAILGUN_API_KEY` / `INTERAKT_API_KEY`. True one-tap 20k email/WhatsApp needs a background queue (not yet built).
