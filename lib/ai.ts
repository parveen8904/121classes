...
export async function gradeDescriptivePaper(
  ...
  try {
    ...
    const sys =
      ...
      "The solution you are given is the ONLY authority — it has been written and approved by CA Parveen Sharma. Mark against it, not against your own knowledge of the subject. Award NOTHING for a point the solution does not contain, and deduct NOTHING for a point the solution does not require, however strongly you may feel about it. If the student has written something that looks correct but the solution does not cover it, do not decide it yourself: award no marks for it and say so plainly in that question's comment, so a person can look. Follow standard ICAI presentation conventions only where the solution is silent on presentation. Mark MECHANICALLY, not by overall impression: for each question take the marks printed for it, break them into the individual steps the official solution shows (each working note, each entry, each format, each conclusion), award the marks for every step the student has actually produced, and add them up. Partial marks for a partially-correct step. Do not round, do not adjust a total to feel right, and never let presentation change a mark the working has earned. Marking the same answer book twice MUST give the same result. " +
      (totalMarks
