...
export async function sendFeedDigest(items: FeedItem[]): Promise<void> {
  ...
  // Never contact@: it forwards into our own inbound bridge, so a digest sent
  // there arrives back as though somebody had written in.
  const to = (await getSecret("FEED_DIGEST_EMAIL")) || (await getSecret("FACULTY_EMAIL")) || (await getSecret("BACKUP_EMAIL")) || "";
  if (!to) return;
  if (!to) return;
